...
import { MapPin, Navigation, Maximize2, ExternalLink, LocateFixed, Loader } from 'lucide-react';
...
function createActiveMarkerIcon(color: string, label: string): L.DivIcon {
  return L.divIcon({
    ...
    <div ref={mapContainerRef} className="card" style={{ padding: 24 }}>
      <div className="section-header">
        ...
        <div style={{ display: 'flex', gap: 4 }}>
          ...
          <button className="icon-btn" title="Fit all markers" onClick={handleFitAll}>
            <Maximize2 size={16} />
          </button>
        </div>
      </div>
        ...
        <div className="map-container">
          ...
          >
            ...
            {mappableEntries.map((entry) => {
              ...
              const icon = isActive
                ? createActiveMarkerIcon(meta.color, meta.icon)
                : createMarkerIcon(meta.color, meta.icon);
